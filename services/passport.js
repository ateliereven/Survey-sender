...
}, async (accessToken, refreshToken, profile, done) => { // taking back the information from google
    const existingUser = await User.findOne({ googleId: profile.id }); // query - look in the User collection and find an item with this profile.id. returns a promise
    if (existingUser) {
        //we already have a record with the given profile.id
        return done(null/*no errors*/, existingUser); // done is a function indicating the operatin has finished and the authentication process can proceed
    }
    //we dont have a record with thid id, make a new record
    const user = await new User({ googleId: profile.id/*the id coming from the user's google profile*/ }).save(); // creating a model instance and saving it to the database. asyncronous operation
    done(null, user);
